...
void read_buttons_with_debounce(void)
	...
	if (!(current_state) == !(button_states & (1 << i)))
		...
		if (!current_state) //aktif low, jika tombol ditekan
		    {
...
int main(void)
    ...
    MX_USB_DEVICE_Init();
    /* USER CODE BEGIN 2 */
    FEE_ReadData(TIPE_EMULASI_ADDRESS, &tipe_emulasi, sizeof(uint8_t));
    FEE_ReadData(AUTO_COUNTER_DELAY_ADDRESS, &auto_counter_delay,
	    sizeof(uint8_t));
    FEE_ReadData(VERBOSE_DELAY_ADDRESS, &verbose_delay, sizeof(uint8_t));
    FEE_ReadData(ARAH_AUTO_COUNTER_ADDRESS, &arah_auto_counter,
	    sizeof(uint8_t));
    /* USER CODE END 2 */
...
// tampilkan verbose button
	if (tipe_emulasi != 4)
		...
		printf("B%d,L%d,C%d,E%d\r\n", button_states, latch_states,
			nilai_emulasi_counter, tipe_emulasi_address_read);
		}
	    }
	else
	    {
...
void USB_CDC_RxHandler(uint8_t *Buf, uint32_t Len)
	...
	if (token != NULL && strcmp(token, "$1") == 0)
		...
		FEE_WriteData(TIPE_EMULASI_ADDRESS, &tipe_emulasi,
			sizeof(uint8_t));
		...
		FEE_WriteData(AUTO_COUNTER_DELAY_ADDRESS, &auto_counter_delay,
			sizeof(uint8_t));
		/* Kirim konfirmasi kembali ke PC */
		char response[64];
		snprintf(response, sizeof(response),
		...
		FEE_WriteData(VERBOSE_DELAY_ADDRESS, &verbose_delay,
			sizeof(uint8_t));
		/* Kirim konfirmasi kembali ke PC */
		char response[64];
		snprintf(response, sizeof(response),
		...
		FEE_WriteData(ARAH_AUTO_COUNTER_ADDRESS, &arah_auto_counter,
			sizeof(uint8_t));
		/* Kirim konfirmasi kembali ke PC */
		char response[64];
		snprintf(response, sizeof(response),
			"arah auto counter diatur ke %d\n", arah_auto_counter);
		CDC_Transmit_FS((uint8_t*) response, strlen(response));
		}
	    }
	else
...
//
///* USER CODE BEGIN PV */
//volatile int32_t pulse_count_servo = 0; // Variabel untuk menyimpan jumlah pulse
//volatile uint8_t direction_servo = 0;
//uint8_t TxBuffer[] =
...
//
//    /* USER CODE BEGIN 1 */
////	uint8_t buff_teks[128]="";
////	uint8_t buff_teks_len = sizeof(buff_teks);
//    /* USER CODE END 1 */
//
//    /* MCU Configuration--------------------------------------------------------*/
//
...
//    while (1)
//	{
//	// Print data pulse dan arah
////		sprintf((char*) buff_teks, "%ld, %d\n", pulse_count_servo,direction_servo);
//	printf("%ld, %d\n", pulse_count_servo, direction_servo);
////		CDC_Transmit_FS(buff_teks, buff_teks_len);
////		HAL_UART_Transmit(&huart2, buff_teks, strlen((char*) buff_teks),HAL_MAX_DELAY);
//	HAL_Delay(100); // Cetak setiap 1 detik
...
//
///* USER CODE BEGIN 4 */
////program untuk interrupt
//void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
//    {
//    if (GPIO_Pin == INT_X_Pin) // jika interrupt pada dir pin
//	{
